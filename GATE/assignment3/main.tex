...
\begin{enumerate}
    ...
    \item \input{fig/1.tex}
    \item \input{fig/2.tex}
    \item \input{fig/3.tex}
    
    \item \input{fig/10.tex}
\end{enumerate}
\end{multicols}
...
\end{enumerate}
\item Consider the circuit shown in the figure, where $RC = 1$. For an input signal $V_1$ shown below, choose the correct $V_o$ from the options: \hfill{$\sbrak{\text{GATE 2015}}$} \\
\begin{figure}[!ht]
\centering
...
\caption{The cuurent $I_0 \brak{\text{in $mA$}}$ is \rule{1.7cm}{0.2mm} $\brak{\text{upto two decimal places}}$ \hfill{$\sbrak{\text{GATE 2015}}$} } 
\end{figure}
